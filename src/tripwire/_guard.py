...
def allow(*rules: str | M) -> Generator[None, None, None]:
    ...
    Usage:
        ...
        # Mixed:
        with tripwire.allow("dns", M(protocol="http", host="*.example.com")):
            ...
    """
    if not rules:
...
def deny(*rules: str | M) -> Generator[None, None, None]:
    ...
    Usage:
        with tripwire.allow("redis"):
            with tripwire.deny(M(protocol="redis", command="FLUSHALL")):
