...
_tripwire_httpx_handle: Callable[..., Any] | None = None
_tripwire_httpx_async_handle: Callable[..., Any] | None = None
_tripwire_requests_send: Callable[..., Any] | None = None
_tripwire_aiohttp_request: Callable[..., Any] | None = None
...
def _find_http_plugin(verifier: "StrictVerifier") -> "HttpPlugin":
    for plugin in verifier._plugins:
        if isinstance(plugin, HttpPlugin):
            return plugin
    raise RuntimeError(
        "BUG: tripwire HttpPlugin interceptor is active but no HttpPlugin "
...
class HttpPlugin(BasePlugin):
    ...
    def install_patches(self) -> None:
        global _tripwire_httpx_handle, _tripwire_httpx_async_handle, _tripwire_requests_send
        global _tripwire_aiohttp_request
        ...
        _tripwire_httpx_handle = _sync_interceptor
        _tripwire_httpx_async_handle = _async_interceptor
        _tripwire_requests_send = _requests_interceptor
    ...
    def _install_urllib(self) -> None:
        ...
        opener = urllib.request.build_opener(_TripwireHandler)
    ...
    def _install_aiohttp(self) -> None:
        ...
        _tripwire_aiohttp_request = _aiohttp_interceptor
    ...
    def _execute_urllib_pass_through(
        ...
        original_opener = HttpPlugin._original_urllib_opener
        # Restore original opener temporarily, make the real request, then reinstall tripwire's
        urllib.request.install_opener(original_opener)
        try:
            response: urllib.response.addinfourl = urllib.request.urlopen(req)
        finally:
            # Reinstall tripwire's opener regardless of outcome
            from tripwire.plugins.http import HttpPlugin as _Self  # noqa: PLC0415
    ...
    def _reinstall_urllib_opener(cls) -> None:
        """Reinstall tripwire's urllib opener after a pass-through call."""
        ...
        class _TripwireHandler(urllib.request.BaseHandler):
            ...
            def http_open(self, req: urllib.request.Request) -> urllib.response.addinfourl:
                return _tripwire_urllib_dispatch_ref(req)

            def https_open(self, req: urllib.request.Request) -> urllib.response.addinfourl:
                return _tripwire_urllib_dispatch_ref(req)

        def _tripwire_urllib_dispatch_ref(
            req: urllib.request.Request,
        ) -> urllib.response.addinfourl:
            url = req.full_url
            method = (req.get_method() or "GET").upper()
